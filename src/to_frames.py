...
#     m = [x1 if x1 < x2 else x2 for x1, x2 in zip(m, np.min(img_norm, axis=(0, 1)).tolist())]
#     M = [x1 if x1 > x2 else x2 for x1, x2 in zip(M, np.max(img_norm, axis=(0, 1)).tolist())]
...
for f in tqdm(files):
    ...
    img_norm = Image.fromarray(img_norm.astype(np.uint8))
    
    plt.imshow(img_norm)
    date = extract_date(f)
    plt.savefig(f"./imgs/{AREA}_{date}.png")
    plt.close()
